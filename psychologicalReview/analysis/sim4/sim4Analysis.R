...
setwd("C:/Users/bentod2/Documents/projects/current/NEWgergliuSims/cognition/data/sim4")
setwd("C:/Users/detbe/Documents/projects/NEWgergliuSims/cognition/data/sim3")
...
D$trialType = rep(c("High", "Low"), each = 1, times = 1200)
...
expTest = t.test(D$lookingTime[D$condition=="Experimental condition" & D$trialType=="High"],
                 D$lookingTime[D$condition=="Experimental condition" & D$trialType=="Low"],
                 paired = TRUE)
expTest
expHighMean = mean(D$lookingTime[D$condition=="Experimental condition" & D$trialType=="High"])
expHighSD = sd(D$lookingTime[D$condition=="Experimental condition" & D$trialType=="High"])
expHighMean
expHighSD

expLowMean = mean(D$lookingTime[D$condition=="Experimental condition" & D$trialType=="Low"])
expLowSD = sd(D$lookingTime[D$condition=="Experimental condition" & D$trialType=="Low"])
expLowMean
expLowSD

# control condition
controlTest = t.test(D$lookingTime[D$condition=="Control condition" & D$trialType=="High"],
                     D$lookingTime[D$condition=="Control condition" & D$trialType=="Low"],
                     paired = TRUE)
controlTest
controlHighMean = mean(D$lookingTime[D$condition=="Control condition" & D$trialType=="High"])
controlHighSD = sd(D$lookingTime[D$condition=="Control condition" & D$trialType=="High"])
controlHighMean
controlHighSD

controlLowMean = mean(D$lookingTime[D$condition=="Control condition" & D$trialType=="Low"])
controlLowSD = sd(D$lookingTime[D$condition=="Control condition" & D$trialType=="Low"])
controlLowMean
controlLowSD

# difference analysis
difExp = D$lookingTime[D$condition=="Experimental condition" & D$trialType=="High"]-
  D$lookingTime[D$condition=="Experimental condition" & D$trialType=="Low"]

difCntrl = D$lookingTime[D$condition=="Control condition" & D$trialType=="High"]-
  D$lookingTime[D$condition=="Control condition" & D$trialType=="Low"]
